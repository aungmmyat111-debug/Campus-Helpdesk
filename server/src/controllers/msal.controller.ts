...
export const handleMicrosoftCallback = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Only set default role when creating a new record
    if (!user) {
    ...
    // Issue internal application JWT (standardized on 'id')
    ...
    // Redirect user back to frontend with token and the DB role
    const frontendUrl = process.env.FRONTEND_URL ?? 'http://localhost:3000';
    return res.redirect(`${frontendUrl}/auth/callback?token=${token}&role=${user.role}&userId=${user.id}`);
  } catch (error) {
